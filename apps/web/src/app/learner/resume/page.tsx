...
import ResumePreview from '@/components/learner/resume/ResumePreview'
...
import type { GeneratedResume } from '@/components/learner/resume/ResumePreview'
...
function KpiCard({ icon: Icon, label, value, delta, deltaGood, iconColor, glow }: any) {
  return (
    <div
      className="rs-kpi-card"
      style={{ boxShadow: `inset 0 0 48px ${glow},0 4px 24px rgba(0,0,0,0.35)` }}
    >
      <div className="rs-kpi-top">
        ...
        <span className={`rs-kpi-delta ${deltaGood ? 'good' : 'warn'}`}>{delta}</span>
      </div>
...
/* ───────────────── PDF EXPORT — client-side, A4 PORTRAIT ───────────────────
 *
 * A4 portrait: 210 mm wide × 297 mm tall
 *
 * Steps:
 *  1. Pin node width to 794 px (≈ 210 mm at 96 dpi) so html2canvas renders
 *     the full portrait width regardless of on-screen card size.
 *  2. Capture at 2× scale (192 dpi) for crisp text.
 *  3. Slice canvas into 297-mm-tall pages if resume is longer than one page.
 *  4. Save with jsPDF in portrait orientation.
 *
 * Both libraries are dynamically imported — zero bundle cost until Export PDF
 * is clicked.
 * ─────────────────────────────────────────────────────────────────────────── */

async function exportToPDF(node: HTMLElement, filename: string): Promise<void> {
  ...
  // A4 portrait at 96 dpi → 210 mm × (96 / 25.4) ≈ 794 px wide
  const PORTRAIT_PX = 794

  node.style.background   = '#ffffff'
  node.style.borderRadius = '0'
  node.style.border       = 'none'
  node.style.width        = `${PORTRAIT_PX}px`

  const canvas = await html2canvas(node, {
    scale:           2,           // 2× → 192 dpi, crisp text
    useCORS:         true,
    backgroundColor: '#ffffff',
    logging:         false,
    windowWidth:     PORTRAIT_PX,
  })
  ...
  // ── Page geometry ─────────────────────────────────────────────────────────
  // Standard A4 portrait — jsPDF orientation:'portrait' keeps these as-is.
  const PAGE_W_MM = 210   // mm — portrait short edge (width)
  const PAGE_H_MM = 297   // mm — portrait long  edge (height / page length)

  // The node was pinned to PORTRAIT_PX which represents PAGE_W_MM.
  // canvas.width = PORTRAIT_PX × scale (2) = 1588 px → maps to 210 mm.
  ...
  // ── Slice into pages ──────────────────────────────────────────────────────
  let yOffset   = 0       // mm of image consumed so far
  let remaining = imgH_mm // mm of image left to place

  while (remaining > 0) {
    const sliceH_mm = Math.min(remaining, PAGE_H_MM)
    const yPx       = yOffset   * pxPerMm
    const sliceHPx  = sliceH_mm * pxPerMm

    // Off-screen canvas holding just this page's slice
    const sliceCanvas        = document.createElement('canvas')
    sliceCanvas.width        = canvas.width
    sliceCanvas.height       = Math.ceil(sliceHPx)
    const ctx                = sliceCanvas.getContext('2d')!
    ctx.drawImage(canvas, 0, -yPx)   // shift canvas upward to expose the slice

    const sliceData = sliceCanvas.toDataURL('image/png')

    if (yOffset > 0) pdf.addPage()
    pdf.addImage(sliceData, 'PNG', 0, 0, PAGE_W_MM, sliceH_mm)
...
export default function ResumeBuilderPage() {
  ...
  // Ref forwarded into ResumePreview — html2canvas captures this exact DOM node
  ...
  const generateResume = useCallback(async () => {
    ...
    setGenerated(mockGenerateResume(profile))
    setGenerating(false)
    setProgress(100)
  }, [profile, generating])
  ...
  const handleExportPDF = useCallback(async () => {
    if (!generated || !resumeNodeRef.current || exporting) return
    setExporting(true)
    try {
      const filename = `${generated.name.replace(/\s+/g, '_')}_Resume_${template}.pdf`
      await exportToPDF(resumeNodeRef.current, filename)
    } catch (err) {
  ...
  return (
    <DashboardShell activeHref="/learner/resume">
      ...
      <div className="rs-page-header">
        <div>
          <h1 className="rs-title">📄 AI Resume Builder</h1>
        ...
        <div className="rs-header-actions">
          ...
          {generated && (
            <button
              ...
              disabled={exporting}
              style={{ opacity: exporting ? 0.7 : 1, minWidth: 130 }}
            >
              {exporting
      ...
      <div className="rs-kpi-strip">
        <KpiCard
          icon={Database} label="Profile Data" value={`${profileScore}%`}
          delta="Complete" deltaGood={profileScore > 70}
          iconColor="#6366f1" glow="rgba(99,102,241,0.07)"
        />
        <KpiCard
          icon={FileText} label="Sections"
          value={`${profile.experience.length + profile.education.length}`}
          delta="Available" deltaGood
          iconColor="#a855f7" glow="rgba(124,58,237,0.07)"
        />
        <KpiCard
          icon={Award} label="Skills" value={String(profile.skills.length)}
          delta={profile.skills.length > 6 ? 'Great coverage' : 'Add more'}
          deltaGood={profile.skills.length > 6}
          iconColor="#10b981" glow="rgba(16,185,129,0.07)"
        />
        <KpiCard
          icon={TrendingUp} label="AI Status"
          value={generating ? 'Generating...' : generated ? 'Generated' : 'Ready'}
          delta={generated ? 'Download ready' : 'Generate resume'}
          deltaGood={!!generated}
          iconColor="#f59e0b" glow="rgba(245,158,11,0.06)"
        />
      </div>
      ...
      <div className="rs-section">
        ...
        <button
          ...
          disabled={generating}
        >
          {generating ? 'Generating Resume...' : generated ? 'Regenerate Resume' : 'Generate Resume with AI'}
          <Sparkles size={16} />
        </button>
        ...
        {generating && (
          <div className="rs-progress">
            ...
            <div className="rs-score-bar">
              <div className="rs-score-fill" style={{ width: `${progress}%` }} />
            </div>
          </div>
        )}
      </div>
