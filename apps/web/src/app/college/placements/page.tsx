...
export default function PlacementsPage() {
  ...
  const avgPkg = Math.round(
    ALL_PLACEMENTS.reduce((a, p) => a + parseInt(p.package), 0) / totalPlaced
  )
      ...
      <motion.div {...fadeUp(1)} className="cl-kpi-strip cl-kpi-strip--4" style={{ marginBottom: 16 }}>
        {[
          ...
          <motion.div
            ...
            className="cl-kpi"
            style={{
              boxShadow: `inset 0 0 40px ${color}09,0 4px 20px rgba(0,0,0,0.32),inset 0 1px 0 rgba(255,255,255,0.05)`
            }}
          >
              ...
              <BarChart data={COMPANY_STATS} layout="vertical" margin={{ top: 4, right: 16, bottom: 4, left: 60 }}>
                ...
                <Tooltip
                  ...
                  formatter={(value) => {
                    ...
                    return [`${Number(rawValue ?? 0)} LPA`, 'Package'];
                  }}
                />
              ...
              {MOCK_BRANCH_STATS.filter(b => b.placed > 0).map(b => (
                <div key={b.branch}>
                  ...
                  <div className="cl-bar cl-bar--md">
                    <motion.div
                      className="cl-bar__fill"
                      style={{ background: b.color, width: `${Math.round(b.placed / b.total * 100)}%` }}
                      initial={{ width: 0 }}
                      animate={{ width: `${Math.round(b.placed / b.total * 100)}%` }}
