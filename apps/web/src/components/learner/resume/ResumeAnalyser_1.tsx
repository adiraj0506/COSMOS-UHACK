...
import { Sparkles, RefreshCw, CheckCircle2, AlertCircle, Info } from 'lucide-react'
...
interface ResumeAnalyserProps {
  resume: GeneratedResume | null   // ← was resumeData: ResumeData
}

// ── Mock analysis ─────────────────────────────────────────────────────────────
// TODO backend: POST /api/resume/analyse { resume: GeneratedResume, jd?: string }
function mockAnalyse(data: GeneratedResume, jd: string): AnalysisResult {
...
export default function ResumeAnalyser({ resume }: ResumeAnalyserProps) {
  ...
  async function runAnalysis() {
    // Guard: nothing to analyse if resume hasn't been generated yet
    if (!resume) return
  ...
  if (!resume) {
    return (
      <div className="rs-card rs-card--cyan h-full flex flex-col">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <p className="rs-label" style={{ marginBottom: 0 }}>
            <Sparkles size={12} style={{ color: '#06b6d4' }} />
            AI Analyser
          </p>
          <button className="rs-btn rs-btn--cyan rs-btn--sm" disabled style={{ opacity: 0.4 }}>
        ...
        <div style={{
          ...
          <p style={{ fontSize: 12, color: '#475569', textAlign: 'center', lineHeight: 1.6 }}>
            Generate a resume first,<br />then analyse it with AI
          </p>
        </div>
      </div>
    )
                ...
                >
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginTop: 14 }}>
                    {[
                      ...
                      { v: result.jdMatch > 0 ? `${result.jdMatch}%` : '—',                                      l: 'JD Match',       c: '#a855f7' },
                    ].map(({ v, l, c }) => (
